...
export async function renderPaymentSummary() {
  ...
  const response1 = await fetch (
    `${window.APP_CONFIG.api.baseUrl}/getcartcount-cdk`,
    {
      method:"POST",
      headers: {
...
for (const item of result1){
  ...
  const response2 = await fetch (
    `${window.APP_CONFIG.api.baseUrl}/products-cdk`,
    {
      method:"POST",
      headers: {
        "Authorization": "Bearer " + idToken,
 ...
 html += `<h3 class="heading-order"> Order Summary</h3>
           ...
           `;    //this needs to make API call to ricemill/ordes and add the order in the orders table in addition once we get response back then we do window.href to order-confirm.html 
           ...
           placeorder.addEventListener( "click", async() => {
            ...
            const response = await fetch (
              `${window.APP_CONFIG.api.baseUrl}/putorders-cdk?user_id=${payload.sub}`,
